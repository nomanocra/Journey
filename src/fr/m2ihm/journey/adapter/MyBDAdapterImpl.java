...
public class MyBDAdapterImpl implements MyBDAdapter{
	...
	private static final String CREATE_TABLE_EVENEMENT =
			"create table " + TABLE_EVENEMENT + " ("
					...
					+ COL_E_ID_VOYAGE + " integer, "
					
					+ COL_E_LAT + " real, "
					+ COL_E_LON + " real, "
					
					+ COL_E_SECONDES + " integer, "
					+ COL_E_MINUTES + " integer, "
					+ COL_E_HOUR + " integer, "
					+ COL_E_DAY + " integer, "
					+ COL_E_MONTH + " integer, "
					+ COL_E_YEAR + " integer, "
					
					+ COL_E_LIEU + " text not null, "
					+ COL_E_COMMENTAIRE + " text not null, "
					+ COL_E_MEDIA_TYPE + " integer not null, "
...
//-------------------------------------------------------------------------
	...
	public ElementMap getEvenement(long id){
		...
		if(typeMedia.equals("Video")){
			e = new Video(v, 
							...
							new Date(cursor.getInt(4), cursor.getInt(5), cursor.getInt(6), cursor.getInt(7), cursor.getInt(8), cursor.getInt(9)), 
			cursor.getString(11),  
			cursor.getString(12), 
			cursor.getString(10));
	...
	public ArrayList<ElementMap> getAllMedia(int idVoyage){
		...
		Cursor cursor = mDB.query(
				...
				new String [] {
						...
						COL_E_ID_VOYAGE + " = " + idVoyage + " AND " + COL_E_MEDIA_TYPE + " <> " + "None",
						null, null, null, null );
		cursor.moveToFirst();
		Voyage v = getVoyage(cursor.getInt(1));
		String typeMedia = cursor.getString(13);
	...
	public ArrayList<Son> getAllSon(int idVoyage){
		...
		Cursor cursor = mDB.query(
				...
				new String [] {
						...
						COL_E_MEDIA_NOM,		//12
						COL_E_MEDIA_TYPE},		
						COL_E_ID_VOYAGE + " = " + idVoyage + 
						" AND " + COL_E_MEDIA_TYPE + " = " + "song",
						null, null, null, null );
		cursor.moveToFirst();
	...
	public ArrayList<ElementMap> getAllPhoto(int idVoyage){
		...
		Cursor cursor = mDB.query(
				...
				new String [] {
						...
						COL_E_MEDIA_NOM},		//13
						COL_E_ID_VOYAGE + " = " + idVoyage + 
						" AND " + COL_E_MEDIA_TYPE + " = " + "photo",
						null, null, null, null );
		cursor.moveToFirst();
		Voyage v = getVoyage(cursor.getInt(1));
		for(int i=0; i<cursor.getCount();i++){
			ElementMap e = new Photo(
										v, 
										new Gps(cursor.getFloat(2), cursor.getFloat(3)), 
										new Date(cursor.getInt(4), cursor.getInt(5), cursor.getInt(6), cursor.getInt(7), cursor.getInt(8), cursor.getInt(9)), 
										cursor.getString(11),  
	...
	public long ajouterElementMap(ElementMap e){
		...
		ContentValues values = new ContentValues();
		values.put(COL_E_ID_VOYAGE, e.getVoyage().getId());
